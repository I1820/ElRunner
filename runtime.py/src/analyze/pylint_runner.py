...
def pylint_check(path, output_format, ignore=None, msg_template=None):
    ...
    pylint docs can be found here:
    `Pylint Docs <https://docs.pylint.org/en/1.6.0/run.html>`_
    :param path: code files path
    :param output_format: output messages format that should
    be OutputFormat class member
...
def run_pylint(path=None, ignore=None, msg_template=None, log_file_path=None):
    """
    Main function to analyze code which returns status code and logs messages
    in the given file
    :param path: code files path
    :param ignore: files or directory(<file[,file...]>) to ignore
